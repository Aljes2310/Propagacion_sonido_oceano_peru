...
ds_sound_profile=ds_sound_profile.interp(depth=new_depths, latitude=np.arange(-67,1.46,0.25), longitude=np.arange(-85,-69, 0.25))
#pasando a df
ds_sound_profile=ds_sound_profile.to_dataframe(name="Sound Velocity(m/s)").dropna().reset_index()

ds_sound_profile["latitude"]=ds_sound_profile['latitude'].astype('float64').round(decimals=4) 
ds_sound_profile["longitude"]=ds_sound_profile['longitude'].astype('float64').round(decimals=4) 
